...
export const postService = {
    ...
    async getTopPosts(limit = 10) {
        ...
        const ids = await redis.zrevrange('posts:top', 0, limit - 1);
        if (!ids.length)
            return [];
        ...
        ids.forEach((id) => pipeline.hmget(`post:${id}`, 'likes', 'views'));
        ...
        for (const [index, id] of ids.entries()) {
            ...
            if (Array.isArray(data)) {
                likes = parseInt(data[0] ?? '0', 10);
                views = parseInt(data[1] ?? '0', 10);
            }
            result.push({
                ...post,
                likes,
